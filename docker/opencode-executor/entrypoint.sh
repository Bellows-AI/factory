...
if [ -n "${ATLASSIAN_SITE:-}" ] && [ -n "${ATLASSIAN_EMAIL:-}" ] && [ -n "${ATLASSIAN_API_TOKEN:-}" ]; then
    printf '%s' "$ATLASSIAN_API_TOKEN" \
        ...
        || echo "opencode-executor: acli could not sign in to $ATLASSIAN_SITE; Jira is unavailable this run" >&2
elif [ -n "${ATLASSIAN_SITE:-}${ATLASSIAN_EMAIL:-}${ATLASSIAN_API_TOKEN:-}" ]; then
    echo "opencode-executor: acli sign-in needs ATLASSIAN_SITE, ATLASSIAN_EMAIL and ATLASSIAN_API_TOKEN; skipped" >&2
fi
