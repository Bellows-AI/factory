...
| `PUBLIC_URL` | loopback origin when possible | External origin used in GitHub redirects. |
...
| `GITHUB_WEBHOOK_SECRET` | unset | HMAC secret enabling the installation webhook route. |
...
| `RUNNER_CLI` | `claude-code` | `claude-code` or `opencode`. |
| `EXECUTOR_IMAGE` | CLI-specific | Runner image name. |
